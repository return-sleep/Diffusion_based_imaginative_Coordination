...
def get_tokenizer(model_name,device='cuda'):
    ...
    if model_type == "I":
        encoder = ImageTokenizer(checkpoint_enc=checkpoint_enc,device=device)
        decoder = ImageTokenizer(checkpoint_dec=checkpoint_dec,device=device)
    elif model_type == "V":
        encoder = CausalVideoTokenizer(checkpoint_enc=checkpoint_enc,device=device)
        decoder = CausalVideoTokenizer(checkpoint_dec=checkpoint_dec,device=device)
...
class ACTPolicyDiffusion_with_Token_Prediction(nn.Module):
    def __init__(self, args_override):
        ...
        self.tokenizer_enc, self.tokenizer_dec = get_tokenizer(
            args_override["tokenizer_model_name"], device='cuda'
        )
        self.tokenizer_enc = nn.DataParallel(self.tokenizer_enc)
    ...
    def train_model(self, qpos, image, env_state, actions, is_pad, is_image_pad=None, task_emb = None):
        ...
        # print('policy/ACT-DP-TP/policy.py task_emb', task_emb.shape)
        env_state = None
        bsz = actions.shape[0]
        ...
        image_tokens = image_tokens.to(qpos.device) 
    ...
    def __call__(
        ...
        task_emb = None,
        save_rec=False
    ): # add task_emb  batch text_dim or None
        env_state = None
        
        # print('policy.py task_emb', task_emb.shape)
        if actions is not None:  # training time
            ...
            task_emb = torch.zeros(all_image.shape[0], 768).to(all_image.device) if task_emb is None else task_emb # Test
            loss_dict, (current_image_tokens, target_token, pred_token) = (
                self.train_model(
                    qpos, all_image, env_state, actions, is_pad, is_pad_img, task_emb
                )
            )  # B H
            ...
            qpos = qpos  # B 1 D
            image = image  # B 1 N C H W task_emb = B D 
            # print(image.shape, image.max(), image.min())
            a_hat, pred_token, _, _ = self.conditional_sample(qpos, image, task_emb)
    ...
    def get_visual_token(self, input_tensor):
        ...
        for view_idx in range(
            ...
            if self.tokenizer_model_type == "DV":  # encoder for video tokenization
                (indices, codes) = self.tokenizer_enc.module._enc_model(
